...
func slashCommand(discord *discordgo.Session, interation *discordgo.InteractionCreate) {
	...
	case "wipe":
		...
		guild_memories[interation.GuildID][args[0].StringValue()] = ""
		discord.InteractionRespond(interation.Interaction, &discordgo.InteractionResponse{
			Type: discordgo.InteractionResponseChannelMessageWithSource,
			Data: &discordgo.InteractionResponseData{
				Content: "`" + args[0].StringValue() + " memory wiped`",
			},
		})
	case "set":
		args := interation.ApplicationCommandData().Options
	...
	case "ai":
		...
		if user_personality[interation.Member.User.ID] == "" {
		...
		response, err := client.Models.GenerateContent(ctx, personality.ai_model, genai.Text(interation.Member.User.GlobalName+" says: "+args[0].StringValue()), &genai.GenerateContentConfig{SystemInstruction: &genai.Content{Parts: []*genai.Part{genai.NewPartFromText(": SYSTEM PROMPT : " + personality.system_prompt + " : MEMORY : " + guild_memories[interation.GuildID][interation.Member.User.ID])}}, Temperature: &personality.temperature})
		if err != nil {
